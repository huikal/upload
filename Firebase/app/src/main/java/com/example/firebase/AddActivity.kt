...
class AddActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // ThirdActivity에서 넘어온 경우 지역 선택 스피너를 보이게 함
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if (item.itemId === R.id.menu_add_gallery) {
            ...
            if (binding.addImageView.drawable !== null && binding.addEditView.text.isNotEmpty() && binding.addArticleView.text.isNotEmpty() && (activityName != "ThirdActivity" || selectedRegion != null)) {
                saveStore()
            } else {
    ...
    private fun saveStore() {
        val data = mapOf(
            ...
            "article" to binding.addArticleView.text.toString(),
            "date" to dateToString(Date()),
            "activityName" to activityName,
            "region" to selectedRegion, // 선택된 지역을 데이터에 포함
            ...
            "meet" to binding.addMeetView.text.toString()
